...
struct kabaneriUnatoViewBayes: View {
    ...
    var body: some View {
        ...
        // //// バッジのリセット
        .resetBadgeOnAppear($common.kabaneriUnatoMenuBayesBadge)
        // //// firebaseログ
        .onAppear {
    ...
    private func bayesRatio() -> [Double] {
        // 下段ベル
        var logPostLowerBell: [Double] = [Double](repeating: 0, count: self.settingList.count)
        if self.lowerBellEnable {
            logPostLowerBell = logPostDenoBino(
                ratio: kabaneriUnato.ratioLowerBell,
