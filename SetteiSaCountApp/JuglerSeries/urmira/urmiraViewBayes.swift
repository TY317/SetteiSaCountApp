...
struct urmiraViewBayes: View {
    @ObservedObject var urmira: Urmira
    
    // 機種ごとに見直し
    let settingList: [Int] = [1,2,3,4,5,6]   // その機種の設定段階
    let payoutList: [Double] = [97.0, 98.1, 99.8, 102.1, 104.5, 108.1]
    @State var bigEnable: Bool = true
    @State var bigDetailEnable: Bool = false
    @State var regEnable: Bool = true
    @State var regDetailEnable: Bool = false
    @State var bellEnable: Bool = true
    @State var cherryEnable: Bool = true
...
//                }
                ...
                // チェリー
                unitToggleWithQuestion(enable: self.$cherryEnable, title: "🍒確率")
            }
            // //// STEP3
        ...
        // //// バッジのリセット
//        .resetBadgeOnAppear($ver391.urmiraMenuBayesBadge)
        // //// firebaseログ
        .onAppear {
    ...
    private func bayesRatio() -> [Double] {
        ...
        if self.bellEnable {
            ...
            if urmira.startBackCalculationEnable {
                logPostBell = logPostDenoBino(
                    ratio: urmira.denominateListBell,
        ...
        // チェリー
        var logPostCherry = [Double](repeating: 0, count: self.settingList.count)
        if self.cherryEnable {
            logPostCherry = logPostDenoBino(
                ratio: urmira.denominateListCherry,
