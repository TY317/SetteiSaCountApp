...
struct goJug3ViewShimaData: View {
    ...
    @AppStorage("goJug3ShimaGameJSON") var shimaGameJSON: String = "[0,0,0,0,0]"
    @AppStorage("goJug3ShimaBigJSON") var shimaBigJSON: String = "[0,0,0,0,0]"
    @AppStorage("goJug3ShimaRegJSON") var shimaRegJSON: String = "[0,0,0,0,0]"
...
//                            goJug3(
            ...
            Section {
                ...
                unitNaviLink95Ci(
                    Ci95view: AnyView(
                        goJug3Ver2View95CiShima(goJug3: goJug3)
                    )
                )
            } header: {
                Text("合計")
        ...
        .onChange(of: self.shimaGames) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                goJug3.shimaGames = shimaGames.reduce(0, +)
            }
        }
        .onChange(of: self.shimaBigs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                goJug3.shimaBigs = shimaBigs.reduce(0, +)
                goJug3.shimaBonusSum = goJug3.shimaBigs + goJug3.shimaRegs
            }
        }
        .onChange(of: self.shimaRegs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                goJug3.shimaRegs = shimaRegs.reduce(0, +)
                goJug3.shimaBonusSum = goJug3.shimaBigs + goJug3.shimaRegs
            }
        }
    }
