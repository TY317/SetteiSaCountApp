...
struct girlsSSViewShimaData: View {
    ...
    @AppStorage("girlsSSShimaGameJSON") var shimaGameJSON: String = "[0,0,0,0,0]"
    @AppStorage("girlsSSShimaBigJSON") var shimaBigJSON: String = "[0,0,0,0,0]"
    @AppStorage("girlsSSShimaRegJSON") var shimaRegJSON: String = "[0,0,0,0,0]"
...
//                            girlsSS(
            ...
            Section {
                ...
                unitNaviLink95Ci(
                    Ci95view: AnyView(
                        girlsSSVer2View95CiShima(girlsSS: girlsSS)
                    )
                )
            } header: {
                Text("合計")
        ...
        .onChange(of: self.shimaGames) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                girlsSS.shimaGames = shimaGames.reduce(0, +)
            }
        }
        .onChange(of: self.shimaBigs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                girlsSS.shimaBigs = shimaBigs.reduce(0, +)
                girlsSS.shimaBonusSum = girlsSS.shimaBigs + girlsSS.shimaRegs
            }
        }
        .onChange(of: self.shimaRegs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                girlsSS.shimaRegs = shimaRegs.reduce(0, +)
                girlsSS.shimaBonusSum = girlsSS.shimaBigs + girlsSS.shimaRegs
            }
        }
    }
