...
struct happyJugV3ViewShimaData: View {
    ...
    @AppStorage("happyJugV3ShimaGameJSON") var shimaGameJSON: String = "[0,0,0,0,0]"
    @AppStorage("happyJugV3ShimaBigJSON") var shimaBigJSON: String = "[0,0,0,0,0]"
    @AppStorage("happyJugV3ShimaRegJSON") var shimaRegJSON: String = "[0,0,0,0,0]"
...
//                            happyJugV3(
            ...
            Section {
                ...
                unitNaviLink95Ci(
                    Ci95view: AnyView(
                        happyJugV3Ver2View95CiShima(happyJugV3: happyJugV3)
                    )
                )
            } header: {
                Text("合計")
        ...
        .onAppear {
            ...
            logEventFirebaseScreen(
                screenName: happyJugV3.machineName,
        ...
        .onChange(of: self.shimaGames) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                happyJugV3.shimaGames = shimaGames.reduce(0, +)
            }
        }
        .onChange(of: self.shimaBigs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                happyJugV3.shimaBigs = shimaBigs.reduce(0, +)
                happyJugV3.shimaBonusSum = happyJugV3.shimaBigs + happyJugV3.shimaRegs
            }
        }
        .onChange(of: self.shimaRegs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                happyJugV3.shimaRegs = shimaRegs.reduce(0, +)
                happyJugV3.shimaBonusSum = happyJugV3.shimaBigs + happyJugV3.shimaRegs
            }
        }
    }
