...
struct funky2ViewShimaData: View {
    ...
    @AppStorage("funky2ShimaGameJSON") var shimaGameJSON: String = "[0,0,0,0,0]"
    @AppStorage("funky2ShimaBigJSON") var shimaBigJSON: String = "[0,0,0,0,0]"
    @AppStorage("funky2ShimaRegJSON") var shimaRegJSON: String = "[0,0,0,0,0]"
...
//                            funky2(
            ...
            Section {
                ...
                unitNaviLink95Ci(
                    Ci95view: AnyView(
                        funky2Ver2View95CiShima(funky2: funky2)
                    )
                )
            } header: {
                Text("合計")
        ...
        .onChange(of: self.shimaGames) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                funky2.shimaGames = shimaGames.reduce(0, +)
            }
        }
        .onChange(of: self.shimaBigs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                funky2.shimaBigs = shimaBigs.reduce(0, +)
                funky2.shimaBonusSum = funky2.shimaBigs + funky2.shimaRegs
            }
        }
        .onChange(of: self.shimaRegs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                funky2.shimaRegs = shimaRegs.reduce(0, +)
                funky2.shimaBonusSum = funky2.shimaBigs + funky2.shimaRegs
            }
        }
    }
