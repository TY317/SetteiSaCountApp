...
struct imJugExViewShimaData: View {
    ...
    @AppStorage("imJugExShimaGameJSON") var shimaGameJSON: String = "[0,0,0,0,0]"
    @AppStorage("imJugExShimaBigJSON") var shimaBigJSON: String = "[0,0,0,0,0]"
    @AppStorage("imJugExShimaRegJSON") var shimaRegJSON: String = "[0,0,0,0,0]"
...
//                            imJugEx(
            ...
            Section {
                ...
                unitNaviLink95Ci(
                    Ci95view: AnyView(
                        imJugExVer2View95CiShima(imJugEx: imJugEx)
                    )
                )
            } header: {
                Text("合計")
        ...
        .onChange(of: self.shimaGames) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                imJugEx.shimaGames = shimaGames.reduce(0, +)
            }
        }
        .onChange(of: self.shimaBigs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                imJugEx.shimaBigs = shimaBigs.reduce(0, +)
                imJugEx.shimaBonusSum = imJugEx.shimaBigs + imJugEx.shimaRegs
            }
        }
        .onChange(of: self.shimaRegs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                imJugEx.shimaRegs = shimaRegs.reduce(0, +)
                imJugEx.shimaBonusSum = imJugEx.shimaBigs + imJugEx.shimaRegs
            }
        }
    }
