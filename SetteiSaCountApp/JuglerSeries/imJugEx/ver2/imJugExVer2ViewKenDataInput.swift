//
//  imJugExVer2ViewKenDataInput.swift
//  SetteiSaCountApp
//
...
struct imJugExVer2ViewKenDataInput: View {
    ...
    var body: some View {
        List {
            Section {
                // ゲーム数入力
                unitTextFieldGamesInput(title: "ゲーム数", inputValue: $imJugEx.kenGameIput)
                    .focused($isFocused)
                    .toolbar {
                ...
                // ビッグ回数
                unitTextFieldGamesInput(title: "BIG回数", inputValue: $imJugEx.kenBigCountInput)
                    .focused($isFocused)
                // レギュラー回数
                unitTextFieldGamesInput(title: "REG回数", inputValue: $imJugEx.kenRegCountInput)
                    .focused($isFocused)
                // ぶどう逆算トグルスイッチ
                Toggle("ぶどう逆算有効化", isOn: $imJugEx.kenBackCalculationEnable)
                // //// 差枚数とぶどう逆算値
                if imJugEx.kenBackCalculationEnable {
                    // 差枚数
                    unitTextFieldGamesInput(
                        title: "差枚数",
                        inputValue: $imJugEx.kenCoinDifferenceInput,
                    ...
                    HStack {
                        ...
                        Text("\(imJugEx.kenBellBackCalculationCount)")
                    ...
                    if orientation.isLandscape || (orientation.isFlat && lastOrientation.isLandscape) {
                        HStack {
                            ...
                            unitResultRatioDenomination2Line(
                                title: "ぶどう確率",
                                count: $imJugEx.kenBellBackCalculationCount,
                            ...
                            unitResultRatioDenomination2Line(
                                title: "BIG確率",
                                count: $imJugEx.kenBigCountInput,
                                bigNumber: $imJugEx.kenGameIput,
                                numberofDicimal: 0,
                                spacerBool: false
                            )
                            ...
                            unitResultRatioDenomination2Line(
                                title: "REG確率",
                                count: $imJugEx.kenRegCountInput,
                                bigNumber: $imJugEx.kenGameIput,
                                numberofDicimal: 0,
                                spacerBool: false
                            )
                            ...
                            unitResultRatioDenomination2Line(
                                title: "ボーナス合算",
                                count: $imJugEx.kenBonusCountSum,
                                bigNumber: $imJugEx.kenGameIput,
                                numberofDicimal: 0,
                                spacerBool: false
                            )
        ...
        .toolbar {
            ToolbarItem(placement: .automatic) {
                HStack {
                    // /// リセット
                    unitButtonReset(isShowAlert: $isShowAlert, action: imJugEx.resetKenDataInput)
