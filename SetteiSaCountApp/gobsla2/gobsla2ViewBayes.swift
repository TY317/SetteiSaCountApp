...
struct gobsla2ViewBayes: View {
    @ObservedObject var gobsla2: Gobsla2
    
    // 機種ごとに見直し
    let settingList: [Int] = [1,2,3,4,5,6]   // その機種の設定段階
    let payoutList: [Double] = [97.6, 98.7, 100.4, 104.9, 109.7, 113.2]
    ...
    var body: some View {
        List {
            ...
            bayesSubStep2Section {
                // 弱レア役からのCZ当選率
                unitToggleWithQuestion(enable: self.$jakuRareEnable, title: "弱レア役からのCZ当選率")
                
                // 300or500GでのCZ当選率
                unitToggleWithQuestion(enable: self.$game35Enable, title: "300・500G到達時のCZ当選率")
        ...
        // //// バッジのリセット
        .resetBadgeOnAppear($common.gobsla2MenuBayesBadge)
        // //// firebaseログ
        .onAppear {
    ...
    private func bayesRatio() -> [Double] {
        // 弱レア役からのCZ当選
        var logPostJakuRare: [Double] = [Double](repeating: 0, count: self.settingList.count)
        if self.jakuRareEnable {
            logPostJakuRare = logPostPercentBino(
                ratio: gobsla2.ratioJakuRareCZ,
        ...
        // 300・500G
        var logpost35: [Double] = [Double](repeating: 0, count: self.settingList.count)
        if self.game35Enable {
            logpost35 = logPostPercentBino(
                ratio: gobsla2.ratio35Hit,
        ...
        // 規定兜pt
        var logPostKabuto: [Double] = [Double](repeating: 0, count: self.settingList.count)
        if self.kabutoEnable {
            logPostKabuto = logPostPercentMulti(
                countList: [
                    gobsla2.ptCount10,
                    gobsla2.ptCount20,
                    gobsla2.ptCount30,
