//
//  draHanaSenkohViewShimaData.swift
//  SetteiSaCountApp
//
...
struct draHanaSenkohViewShimaData: View {
    ...
    @AppStorage("draHanaSenkohShimaGameJSON") var shimaGameJSON: String = "[0,0,0,0,0]"
    @AppStorage("draHanaSenkohShimaBigJSON") var shimaBigJSON: String = "[0,0,0,0,0]"
    @AppStorage("draHanaSenkohShimaRegJSON") var shimaRegJSON: String = "[0,0,0,0,0]"
...
//                            draHanaSenkoh(
            ...
            Section {
                ...
                unitNaviLink95Ci(
                    Ci95view: AnyView(
                        draHanaSenkohVer2View95CiShima(draHanaSenkoh: draHanaSenkoh)
                    )
                )
            } header: {
                Text("合計")
        ...
        // //// バッジのリセット
        .resetBadgeOnAppear($common.draHanaSenkohMenuShimaBadge)
        // //// firebaseログ
        .onAppear {
        ...
        .onChange(of: self.shimaGames) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                draHanaSenkoh.shimaGames = shimaGames.reduce(0, +)
            }
        }
        .onChange(of: self.shimaBigs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                draHanaSenkoh.shimaBigs = shimaBigs.reduce(0, +)
                draHanaSenkoh.shimaBonusSum = draHanaSenkoh.shimaBigs + draHanaSenkoh.shimaRegs
            }
        }
        .onChange(of: self.shimaRegs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                draHanaSenkoh.shimaRegs = shimaRegs.reduce(0, +)
                draHanaSenkoh.shimaBonusSum = draHanaSenkoh.shimaBigs + draHanaSenkoh.shimaRegs
            }
        }
    }
