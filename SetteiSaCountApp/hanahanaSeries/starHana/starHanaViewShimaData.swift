...
struct starHanaViewShimaData: View {
    ...
    @AppStorage("starHanaShimaGameJSON") var shimaGameJSON: String = "[0,0,0,0,0]"
    @AppStorage("starHanaShimaBigJSON") var shimaBigJSON: String = "[0,0,0,0,0]"
    @AppStorage("starHanaShimaRegJSON") var shimaRegJSON: String = "[0,0,0,0,0]"
...
//                            starHana(
            ...
            Section {
                ...
                unitNaviLink95Ci(
                    Ci95view: AnyView(
                        starHanaVer2View95CiShima(starHana: starHana)
                    )
                )
            } header: {
                Text("合計")
        ...
        // //// バッジのリセット
        .resetBadgeOnAppear($common.starHanaMenuShimaBadge)
        // //// firebaseログ
        .onAppear {
        ...
        .onChange(of: self.shimaGames) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                starHana.shimaGames = shimaGames.reduce(0, +)
            }
        }
        .onChange(of: self.shimaBigs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                starHana.shimaBigs = shimaBigs.reduce(0, +)
                starHana.shimaBonusSum = starHana.shimaBigs + starHana.shimaRegs
            }
        }
        .onChange(of: self.shimaRegs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                starHana.shimaRegs = shimaRegs.reduce(0, +)
                starHana.shimaBonusSum = starHana.shimaBigs + starHana.shimaRegs
            }
        }
    }
