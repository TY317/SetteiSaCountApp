//
//  starHanaViewBayes.swift
//  SetteiSaCountApp
//
//  Created by 横田徹 on 2025/09/11.
//
    ...
    private func bayesRatio() -> [Double] {
        ...
        if self.bigEnable {
            logPostBig = logPostDenoBino(
                ratio: starHana.denominateListBig,
        ...
        if self.regEnable {
            logPostReg = logPostDenoBino(
                ratio: starHana.denominateListReg,
        ...
        if self.bellEnable {
            ...
            if starHana.startBackCalculationEnable {
                logPostBell = logPostDenoBino(
                    ratio: starHana.denominateListBell,
        ...
        if self.bbSuikaEnable {
            logPostBigSuiKa = logPostDenoBino(
                ratio: starHana.denominateListBbSuika,
        ...
        if self.bigLampEnable {
            logPostBigLamp = logPostPercentMulti(
                countList: [
                    starHana.bbLampBCount,
                    starHana.bbLampYCount,
                    starHana.bbLampGCount,
                    starHana.bbLampRCount,
                ], ratioList: [
        ...
        if self.regSideLampEnable {
            logPostRegLamp = logPostPercentMulti(
                countList: [
                    starHana.rbLampBCount,
                    starHana.rbLampYCount,
                    starHana.rbLampGCount,
                    starHana.rbLampRCount,
                ], ratioList: [
                    [36,23,33,22,31,25],
                    [24,35,22,32,21,25],
                    [24,17,27,18,29,25],
