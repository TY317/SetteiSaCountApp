...
struct kingHanaViewShimaData: View {
    ...
    @AppStorage("kingHanaShimaGameJSON") var shimaGameJSON: String = "[0,0,0,0,0]"
    @AppStorage("kingHanaShimaBigJSON") var shimaBigJSON: String = "[0,0,0,0,0]"
    @AppStorage("kingHanaShimaRegJSON") var shimaRegJSON: String = "[0,0,0,0,0]"
...
//                            kingHana(
            ...
            Section {
                ...
                unitNaviLink95Ci(
                    Ci95view: AnyView(
                        kingHanaVer2View95CiShima(kingHana: kingHana)
                    )
                )
            } header: {
                Text("合計")
        ...
        // //// バッジのリセット
        .resetBadgeOnAppear($common.kingHanaMenuShimaBadge)
        // //// firebaseログ
        .onAppear {
        ...
        .onChange(of: self.shimaGames) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                kingHana.shimaGames = shimaGames.reduce(0, +)
            }
        }
        .onChange(of: self.shimaBigs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                kingHana.shimaBigs = shimaBigs.reduce(0, +)
                kingHana.shimaBonusSum = kingHana.shimaBigs + kingHana.shimaRegs
            }
        }
        .onChange(of: self.shimaRegs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                kingHana.shimaRegs = shimaRegs.reduce(0, +)
                kingHana.shimaBonusSum = kingHana.shimaBigs + kingHana.shimaRegs
            }
        }
    }
