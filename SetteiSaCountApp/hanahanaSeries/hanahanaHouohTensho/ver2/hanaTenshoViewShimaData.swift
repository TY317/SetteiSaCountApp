...
struct hanaTenshoViewShimaData: View {
    ...
    @AppStorage("hanaTenshoShimaGameJSON") var shimaGameJSON: String = "[0,0,0,0,0]"
    @AppStorage("hanaTenshoShimaBigJSON") var shimaBigJSON: String = "[0,0,0,0,0]"
    @AppStorage("hanaTenshoShimaRegJSON") var shimaRegJSON: String = "[0,0,0,0,0]"
...
//                            hanaTensho(
            ...
            Section {
                ...
                unitNaviLink95Ci(
                    Ci95view: AnyView(
                        hanaTenshoVer2View95CiShima(hanaTensho: hanaTensho)
                    )
                )
            } header: {
                Text("合計")
        ...
        // //// バッジのリセット
        .resetBadgeOnAppear($common.hanaTenshoMenuShimaBadge)
        // //// firebaseログ
        .onAppear {
        ...
        .onChange(of: self.shimaGames) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                hanaTensho.shimaGames = shimaGames.reduce(0, +)
            }
        }
        .onChange(of: self.shimaBigs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                hanaTensho.shimaBigs = shimaBigs.reduce(0, +)
                hanaTensho.shimaBonusSum = hanaTensho.shimaBigs + hanaTensho.shimaRegs
            }
        }
        .onChange(of: self.shimaRegs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                hanaTensho.shimaRegs = shimaRegs.reduce(0, +)
                hanaTensho.shimaBonusSum = hanaTensho.shimaBigs + hanaTensho.shimaRegs
            }
        }
    }
