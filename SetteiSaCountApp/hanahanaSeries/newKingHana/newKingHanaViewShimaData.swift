//
//  newKingHanaViewShimaData.swift
//  SetteiSaCountApp
//
...
struct newKingHanaViewShimaData: View {
    ...
    @AppStorage("newKingHanaShimaGameJSON") var shimaGameJSON: String = "[0,0,0,0,0]"
    @AppStorage("newKingHanaShimaBigJSON") var shimaBigJSON: String = "[0,0,0,0,0]"
    @AppStorage("newKingHanaShimaRegJSON") var shimaRegJSON: String = "[0,0,0,0,0]"
...
//                            newKingHana(
        ...
        // //// バッジのリセット
        .resetBadgeOnAppear($common.newKingHanaMenuShimaBadge)
        // //// firebaseログ
        .onAppear {
        ...
        .onChange(of: self.shimaGames) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                newKingHana.shimaGames = shimaGames.reduce(0, +)
            }
        }
        .onChange(of: self.shimaBigs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                newKingHana.shimaBigs = shimaBigs.reduce(0, +)
                newKingHana.shimaBonusSum = newKingHana.shimaBigs + newKingHana.shimaRegs
            }
        }
        .onChange(of: self.shimaRegs) { oldValue, newValue in
            if let data = try? JSONEncoder().encode(newValue),
               let str = String(data: data, encoding: .utf8) {
                ...
                newKingHana.shimaRegs = shimaRegs.reduce(0, +)
                newKingHana.shimaBonusSum = newKingHana.shimaBigs + newKingHana.shimaRegs
            }
        }
    }
